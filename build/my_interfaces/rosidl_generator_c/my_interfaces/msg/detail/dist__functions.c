// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from my_interfaces:msg/Dist.idl
// generated code does not contain a copyright notice
#include "my_interfaces/msg/detail/dist__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `dist`
#include "rosidl_runtime_c/string_functions.h"

bool
my_interfaces__msg__Dist__init(my_interfaces__msg__Dist * msg)
{
  if (!msg) {
    return false;
  }
  // dist
  if (!rosidl_runtime_c__String__init(&msg->dist)) {
    my_interfaces__msg__Dist__fini(msg);
    return false;
  }
  return true;
}

void
my_interfaces__msg__Dist__fini(my_interfaces__msg__Dist * msg)
{
  if (!msg) {
    return;
  }
  // dist
  rosidl_runtime_c__String__fini(&msg->dist);
}

bool
my_interfaces__msg__Dist__are_equal(const my_interfaces__msg__Dist * lhs, const my_interfaces__msg__Dist * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // dist
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->dist), &(rhs->dist)))
  {
    return false;
  }
  return true;
}

bool
my_interfaces__msg__Dist__copy(
  const my_interfaces__msg__Dist * input,
  my_interfaces__msg__Dist * output)
{
  if (!input || !output) {
    return false;
  }
  // dist
  if (!rosidl_runtime_c__String__copy(
      &(input->dist), &(output->dist)))
  {
    return false;
  }
  return true;
}

my_interfaces__msg__Dist *
my_interfaces__msg__Dist__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interfaces__msg__Dist * msg = (my_interfaces__msg__Dist *)allocator.allocate(sizeof(my_interfaces__msg__Dist), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(my_interfaces__msg__Dist));
  bool success = my_interfaces__msg__Dist__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
my_interfaces__msg__Dist__destroy(my_interfaces__msg__Dist * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    my_interfaces__msg__Dist__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
my_interfaces__msg__Dist__Sequence__init(my_interfaces__msg__Dist__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interfaces__msg__Dist * data = NULL;

  if (size) {
    data = (my_interfaces__msg__Dist *)allocator.zero_allocate(size, sizeof(my_interfaces__msg__Dist), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = my_interfaces__msg__Dist__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        my_interfaces__msg__Dist__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
my_interfaces__msg__Dist__Sequence__fini(my_interfaces__msg__Dist__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      my_interfaces__msg__Dist__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

my_interfaces__msg__Dist__Sequence *
my_interfaces__msg__Dist__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_interfaces__msg__Dist__Sequence * array = (my_interfaces__msg__Dist__Sequence *)allocator.allocate(sizeof(my_interfaces__msg__Dist__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = my_interfaces__msg__Dist__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
my_interfaces__msg__Dist__Sequence__destroy(my_interfaces__msg__Dist__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    my_interfaces__msg__Dist__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
my_interfaces__msg__Dist__Sequence__are_equal(const my_interfaces__msg__Dist__Sequence * lhs, const my_interfaces__msg__Dist__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!my_interfaces__msg__Dist__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
my_interfaces__msg__Dist__Sequence__copy(
  const my_interfaces__msg__Dist__Sequence * input,
  my_interfaces__msg__Dist__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(my_interfaces__msg__Dist);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    my_interfaces__msg__Dist * data =
      (my_interfaces__msg__Dist *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_interfaces__msg__Dist__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          my_interfaces__msg__Dist__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!my_interfaces__msg__Dist__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
